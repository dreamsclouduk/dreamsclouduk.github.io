...
$('.navbar-toggle-wrapper').click(function (e) {
    e.stopImmediatePropagation()
    $('.navbar-toggle').toggleClass('active')
    $('.navbar-menu').toggleClass('active')
    $('html').toggleClass('overflow-hidden', $('.navbar-menu').hasClass('active'))
})
$('.navbar-menu-link:not(.no-prevent)').click(function (e) {
    e.preventDefault()
    e.stopImmediatePropagation()
    if ($(window).width() < 992) {
        if (!$(this).hasClass('active')) {
            $('.navbar-menu-link.active').removeClass('active')
            $('.navbar-menu-dropdown').slideUp('fast')
            $(this).addClass('active')
            $(this).next('.navbar-menu-dropdown').slideDown('fast')
        } else {
...
$('.listing-search-open').click(function (e) {
    e.preventDefault()
...
$('.listing-search-popup-close').click(function () {
    $('.listing-search-popup').removeClass('active')
...
$('.listing-search-popup-filter-toggle').click(function (e) {
    e.preventDefault()
...
var svgArr = {
    history: '<svg class="sm"> <use xlink:href="/tpl4/index.svg?cache=1&' +
        version +
        '#recent"></use> </svg>', search: '<svg class="sm"> <use xlink:href="/tpl4/index.svg?cache=1&' +
            ...
            '#search"></use> </svg>', location: '<svg class="sm"> <use xlink:href="/tpl4/index.svg?cache=1&' +
                ...
                '#location"></use> </svg>', trending: '<svg class="sm"> <use xlink:href="/tpl4/index.svg?cache=1&' +
                    version +
                    '#trending"></use> </svg>', "no-svg": "",
}; function listingAutocompleteCategory(inp) {
    var selectedId = -1
    inp.on('input focus', function () {
        var url = '/include-functions/category.php?cache=5&noindex=1&search=' + inp.val()
        ...
        $.ajax({
            url: url, success: function (result) {
                ...
                closeAutocomplete()
                selectedId = -1
                var wrapper = $(document.createElement("ul")); wrapper.addClass("listing-search-popup-autocomplete"); for (var i = 0; i < arr.length; i++) {
                    var item = $(document.createElement("li")); if (arr[i].id == "title") { item.addClass("disabled"); }
                    item.attr("data-value", arr[i].cat_name); item.attr("data-text", arr[i].id); item.attr("data-catId", arr[i].cats_id); item.html(svgArr[arr[i].svg]); var subitem = $(document.createElement("span")); subitem.html(arr[i].name); item.append(subitem); item.mousedown(onMouseDownAndClick); item.click(onMouseDownAndClick); function onMouseDownAndClick() {
                        inp.val($(this).attr("data-text")); $('#v1-global').val($(this).attr("data-value"))
...
function listingAutocompleteLocation(inp) {
    var selectedId = -1
    inp.on('input focus', function () {
        var url = '/include-functions/country.php?cache=5&noindex=2&search=' + inp.val()
...
function customSelectRender() {
    $('.custom-select').each(function (i, val) {
        var current = $(val)
        var selected = current.find('.select-option > [data-selected]').eq(0)
        if (selected.length > 0) {
            current.find('.select-value').val(selected.data('value'))
...
$(document).on('click', '.select-option > :not(.select-option-search)', function (e) {
    e.stopImmediatePropagation(); var el = $(this); var customSelect = el.closest('.custom-select')
    customSelect.find('.select-value').val($(this).attr('data-value')).trigger('change')
    customSelect.find('.selected').text($(this).text())
    customSelect.removeClass('active')
})
$(document).click(function (e) { if (!$(e.target).is('.custom-select, .custom-select > *')) { $('.custom-select').removeClass('active') } }); function sliderArrowAction() {
    $('.slider-arrow.prev').click(function () { $(this).siblings('.slider-wrapper').scrollLeft($(this).siblings('.slider-wrapper').scrollLeft() - $(this).siblings('.slider-wrapper').width()) })
...
$('.clear-input').on('click mousedown', function (e) {
    e.preventDefault()
...
function lazyloadFunc() {
    var imageLazy = $('img.lazy, img[src^="https://img.rightbiz.co.uk/listings/"]')
    if ('IntersectionObserver' in window) {
        var imageLazyLoadObserver = new IntersectionObserver(function (entries) {
            $.each(entries, function (i, img) {
        ...
        $.each(imageLazy, function (i, image) { imageLazyLoadObserver.observe(image) })
    } else {
        $.each(imageLazy, function (i, image) {
            if ($(image).hasClass('lazy')) {
                $(image).attr('src', $(image).data('src'))
...
function fixImage(image) {
    if (image.attr('src').startsWith('https://img.rightbiz.co.uk/listings/')) {
        image.on('error', function () {
            var splitted = image.attr('src').split('/')
...
function getUrlParameter(sParam) {
    ...
    return false;
}; $('.listing-search-popup-save').click(function () {
    var $btn = $(this)
    if ($btn.data("state") == "save") { var state = 0 } else { var state = 1 }
    var url = "/ajax/save_search.ajax.php?cacheno=1&noindex=1&headline=Businesses for Sale in UK&location=Nationwide, UK&link=/businesses-for-sale-in-uk.html" + "&state=" + state; gtag("event", "click", { event_category: "ListingPage", event_label: "save-search", non_interaction: false, }); $.ajax({
        url: url, success: function (result) {
            var result = JSON.parse(result); if (result.redirect) { window.location.href = result.redirect; return false; }
            if (result.saved == true) { $btn.data("state", "saved"); $btn.html('Saved') } else { $btn.data("state", "save"); $btn.html('Save Search') }
        }
    })
})
if (!getCookie('rb_user')) {
    // $('.navbar-account').html('<svg class="fill-blue" width="24px" height="24px">\
    //         <use xlink:href="/header-footer2/index.svg#user-circle"></use>\
    //     </svg><span class="text-sm font-medium text-dark">Sign In</span>')
} else {
    // $('.navbar-account').html('<svg class="fill-blue" width="24px" height="24px">\
    //         <use xlink:href="/header-footer2/index.svg#user-circle"></use>\
